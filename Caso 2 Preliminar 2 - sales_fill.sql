	...
	SET copero_counter = 0;
 	WHILE copero_counter < 30 DO
		...
		WHILE sale_counter < sale_range DO
			...
			VALUE
				(copero_counter, "efectivo", CONCAT(cur_date," ",horas), "compra", "exitosa", CONCAT(cur_date," ",horas), "me", "root", SHA2("password",256));
			...
			VALUE
				(last_insert_id_iv, last_insert_id_od, rand_prod_quant * price, CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "me", "root", SHA2("password",256));
            UPDATE invoices iv
            ...
            VALUE
				(CEILING(copero_counter/2), rand_prod_id, rand_prod_quant * 100, CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "venta");
            ...
            VALUE
				(1, 1, copero_counter, NULL, CONCAT(cur_date," ",horas),CONCAT(cur_date," ",horas), "me", "root", SHA2("password",256), rand_prod_quant * (100 + FLOOR(RAND() * 31)), last_insert_id_inv);
                
			INSERT INTO inventories (car_id, ingredient_id, quantity, updated_at, created_at, operation_type)
            VALUE
				(CEILING(copero_counter/2), 11, rand_prod_quant * 200, CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "venta");
			SET last_insert_id_inv = LAST_INSERT_ID();
            
            INSERT INTO inventory_checks (check_status_id, check_type_id, first_copero, second_copero, updated_at, created_at, computer, username, checksum, real_amount, inventory_id)
            VALUE
				(1, 1, copero_counter, NULL, CONCAT(cur_date," ",horas),CONCAT(cur_date," ",horas), "me", "root", SHA2("password",256), rand_prod_quant * (200 + FLOOR(RAND() * 61)), last_insert_id_inv);
                
			INSERT INTO inventories (car_id, ingredient_id, quantity, updated_at, created_at, operation_type)
            VALUE
				(CEILING(copero_counter/2), 12, rand_prod_quant * 50, CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "venta");
			SET last_insert_id_inv = LAST_INSERT_ID();
            
            INSERT INTO inventory_checks (check_status_id, check_type_id, first_copero, second_copero, updated_at, created_at, computer, username, checksum, real_amount, inventory_id)
            VALUE
				(1, 1, copero_counter, NULL, CONCAT(cur_date," ",horas),CONCAT(cur_date," ",horas), "me", "root", SHA2("password",256), rand_prod_quant * (50 + FLOOR(RAND() * 16)), last_insert_id_inv);
                
			INSERT INTO inventories (car_id, ingredient_id, quantity, updated_at, created_at, operation_type)
            VALUE
				(CEILING(copero_counter/2), 13, rand_prod_quant * 25, CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "venta");
			SET last_insert_id_inv = LAST_INSERT_ID();
            
            INSERT INTO inventory_checks (check_status_id, check_type_id, first_copero, second_copero, updated_at, created_at, computer, username, checksum, real_amount, inventory_id)
            VALUE
				(1, 1, copero_counter, NULL, CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "me", "root", SHA2("password",256), rand_prod_quant * (25 + FLOOR(RAND() * 8)), last_insert_id_inv);
			
			INSERT INTO cashboxes (car_id, created_at, updated_at, operation_type, amount)
			VALUE
				(CEILING(copero_counter/2), CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "debito", rand_prod_quant * price);
			...
			VALUES
				(1, 1, copero_counter, NULL, CONCAT(cur_date," ",horas), CONCAT(cur_date," ",horas), "me", "root", SHA2("password",256), rand_prod_quant * price, last_insert_id_cb);
		END WHILE;
	END WHILE;
  END WHILE;
